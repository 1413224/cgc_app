		...
		<div class="wrapper" ref="wrapper">
			<div class="content">
				...
				<div class="store-allbox" v-for="(item,index) in proList" :key="index">
					...
					<div class="pro-box">
						<div :class="{'vux-1px-r':index != 2}" v-for="(i,index2) in item.pro" :key="index2" @click="toStoreDetails(index,i,index2,i.url)">
							<p>{{i.name}}</p>
				...
				<div class="pro-allbox">
					...
					<div class="item-box">
						<div class="item" 
							...
							<p class="price">
								<span class="jg" v-if="item.money">¥ {{(item.money - item.zf).toFixed(2)}}</span>
								<span class="zf" v-if="item.zf">
									<span v-if="item.money"></span>+ {{item.zf}} 
									<span class="f20">信用积分</span>
								</span>
								<span class="img"><img src="../assets/images/shop/tip1.png" alt=""></span>
							</p>
				...
				/*{
					...
					pro: [{
						...
						},
						{
							name: '约派克',
							tip: '约派克 约成功',
							img: './static/index/yuepaike.png',
							id: 'enterBasic620436368300000001'
						}
					]
				}*/
				],
