			...
			<div class="wrapper" ref="wrapper" :class="{'top0':tabNo}">
				<div class="content">
					<div class="order_item" v-for="(item,index) in sList">
						...
						<div class="order_middle" @click="toDetail" v-for="(i,index) in item.pList" :key="index" v-if="(index+1) <= showNum" :class="{'m':index <= showNum - 2}">
							<div class="left">
	...
	export default {
		...
		methods: {
			...
			lookAllp(length) {
				...
				this.showNum = this.showAll ? length : 10
			},
			//付款
			pay() {
				this.$router.push({
